...
def calculate_accumulated_discounted_rewards(episode_rewards, discount_factor):
    # Note this current implementation is slightly buggy in that it does not use the discount factor. 
    # You are asked to re-write this function fully in Lab 10 anyway.
    # discounted_episode_rewards= np.ones_like(episode_rewards)*sum(episode_rewards)# TODO fix this to do the proper calculation for R_t
    ...
    # Base case: if the list is empty, the sum is an empty list
    # Base case: if the list is empty, return an empty NumPy array
    if not episode_rewards:
        return np.array([])
    else:
        # Calculate the accumulated discounted rewards for the rest of the list
        rest_rewards = calculate_accumulated_discounted_rewards(episode_rewards[1:], discount_factor)

        # Calculate the current cumulative discounted reward
        current_reward = episode_rewards[0] + discount_factor * (rest_rewards[0] if rest_rewards.size > 0 else 0)

        # Return the current cumulative discounted reward followed by the rest of the accumulated rewards
        return np.concatenate(([current_reward], rest_rewards))
